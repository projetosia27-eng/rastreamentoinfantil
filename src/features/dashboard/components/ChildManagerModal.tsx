...
import { Smartphone, Watch, Radio, Tag, X, Phone } from 'lucide-react';
...
export default function ChildManagerModal({ onClose, childToEdit }: ChildManagerModalProps) {
  ...
  const [phone, setPhone] = useState(childToEdit?.phone || '');
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col overflow-hidden">
        
        {/* Modal Header */}
        <div className="p-4 px-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between shrink-0 bg-slate-50/50 dark:bg-slate-900">
          <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">
            {childToEdit ? 'Editar Perfil do Filho' : 'Adicionar Nova Criança'}
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="p-5 overflow-y-auto space-y-4 flex-1">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">
                Telefone / Celular do Filho (Opcional)
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 text-xs border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Contato de Emergência dos Pais</label>
              <input
                type="tel"
          ...
          {/* Fixed Bottom Footer with Always Visible Save Button */}
          <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/90 shrink-0 flex gap-2">
            <button
              type="button"
              onClick={onClose}
              disabled={isLoading}
              className="flex-1 py-2.5 text-xs font-semibold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl disabled:opacity-50 transition-colors"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={isLoading}
              className="flex-1 py-2.5 text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl disabled:opacity-50 flex items-center justify-center gap-2 shadow-md shadow-indigo-600/20 active:scale-95 transition-all"
            >
              {isLoading ? (
                <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
              ) : null}
              {childToEdit ? 'Salvar Alterações' : 'Salvar Criança'}
            </button>
          </div>
        </form>
      </div>
